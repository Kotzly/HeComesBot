...
from hecomes.config import config_tuple, load_default_config, load_personality_list
...
def parse_cmd_args():
    ...
    parser.add_option(
        ...
        dest="dims",
        default="512x512",
        help="Image width x height, e.g. 320x240",
    )
    parser.add_option(
        "-s",
        "--seed",
        dest="seed",
        default=int(1000 * time.time()),
        help="Random seed (uses system time by default)",
    )
    parser.add_option(
        "-f",
        "--fontsize",
        dest="fontsize",
        default=40,
        help="Fontsize for text in image.",
    )
    parser.add_option(
        "-D",
        "--default_config",
        ...
        default=False,
        action="store_true",
        help="Use default Configuration file.",
    )
